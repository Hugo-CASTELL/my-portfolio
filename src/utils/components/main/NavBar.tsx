...
function Arrow(props: Readonly<ArrowProps>) {
  return (
    <button onClick={props.action} className="relative">
      <svg
...
export default function NavBar(props: Readonly<NavBarProps>) {
  ...
  const nextPath = (left: boolean) => {
    const currentIndex = props.routes.findIndex(
      (route) => route.path === location.pathname,
    )
    const move = left ? -1 : 1
    return props.routes[
      (currentIndex + move + props.routes.length) % props.routes.length
    ].path
  }
      ...
      <ul className="flex">
        {props.routes.map((route, i) => (
          ...
          >
            ...
            <p
              className={
                "relative z-10 m-4 text-(length:--subtitle-size) font-bold"
              }
            >
              {route.title}
            </p>
            <div
              className={
                `absolute bottom-1/5 left-1/10 z-0 ${gradients[i % gradients.length]} bg-blend-color-dodge h-1 ` +
